    ...
    switch (cheatCode) {
      ...
      case 'power overwhelming':
        if (Cheat.oldCalculateDamageBy) {
          ...
          Cheat.oldCalculateDamageBy = [
            Unit.prototype.calculateDamageBy,
            Building.prototype.calculateDamageBy,
          ];
        ...
        break;
      case 'big daddy':
        var daddy = new Hero.HeroCruiser({
          x: Map.offsetX + Game.HBOUND / 2,
          y: Map.offsetY + Game.VBOUND / 2,
        });
        Game.changeSelectedTo(daddy);
        break;
      case 'big mommy':
        var mommy = new Hero.Sarah({
          x: Map.offsetX + Game.HBOUND / 2,
          y: Map.offsetY + Game.VBOUND / 2,
        });
