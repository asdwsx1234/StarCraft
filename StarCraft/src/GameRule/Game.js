...
import Levels from '../GameRule/Levels';
...
var Game = {
  ...
  addIntoAllSelected: function (chara, override) {
    ...
    Game.allSelected.sort(function (chara1, chara2) {
      ...
      var name1 =
        chara1 instanceof Building
          ? chara1.inherited.name + '.' + chara1.name
          : chara1.name;
      var name2 =
        chara2 instanceof Building
          ? chara2.inherited.name + '.' + chara2.name
          : chara2.name;
  ...
  init: function () {
    ...
    sourceLoader.load(
      'img',
      'img/Charas/ProtossBuilding.png',
      'ProtossBuilding'
    );
    ...
    sourceLoader.load(
      'img',
      'img/Maps/(4)Orbital Relay.jpg',
      'Map_OrbitalRelay'
    );
    sourceLoader.load('img', 'img/Maps/(6)Thin Ice.jpg', 'Map_ThinIce');
    sourceLoader.load(
      'img',
      'img/Maps/(8)BigGameHunters.jpg',
      'Map_BigGameHunters'
    );
  ...
  multiSelectInRect: function () {
    ...
    var startPoint = {
      x:
        Map.offsetX +
        Math.min(mouseController.startPoint.x, mouseController.endPoint.x),
      y:
        Map.offsetY +
        Math.min(mouseController.startPoint.y, mouseController.endPoint.y),
    };
    var endPoint = {
      x:
        Map.offsetX +
        Math.max(mouseController.startPoint.x, mouseController.endPoint.x),
      y:
        Map.offsetY +
        Math.max(mouseController.startPoint.y, mouseController.endPoint.y),
    };
  ...
  getSelectedOne: function (
    ...
    var distance = function (chara) {
      return (
        (clickX - chara.posX()) * (clickX - chara.posX()) +
        (clickY - chara.posY()) * (clickY - chara.posY())
      ); //Math.pow2
    };
    //Initial
    var selectedOne = {},
    ...
    if (isEnemyFilter == null) {
      ...
      if (unitBuildingFilter == null)
        charas = Unit.allEnemyUnits().concat(Building.enemyBuildings);
      else if (unitBuildingFilter) charas = Unit.allEnemyUnits();
      else charas = Building.enemyBuildings;
    } else {
      if (unitBuildingFilter == null)
        charas = Unit.allOurUnits().concat(Building.ourBuildings);
  ...
  getInRangeOnes: function (
    ...
    selectedOnes = charas.filter(function (chara) {
      return (
        chara.status != 'dead' &&
        chara.insideSquare({ centerX: clickX, centerY: clickY, radius: range })
      );
    });
    return selectedOnes;
  },
  ...
  getNearbyOnes: function (clickX, clickY, isEnemyFilter) {
    ...
    var units = [];
    //No filter, all units
    if (isEnemyFilter == undefined)
      units = Unit.allUnits.concat(Building.allBuildings);
    //_$.copy(Unit.allUnits)
    else {
      //Only enemy units
      if (isEnemyFilter)
        units = Unit.allEnemyUnits().concat(Building.enemyBuildings);
      //_$.copy(Unit.allEnemyUnits())
      //Only our units
      else units = Unit.allOurUnits().concat(Building.ourBuildings); //_$.copy(Unit.allOurUnits())
    }
    units.sort(function (unit1, unit2) {
      return (
        (unit1.posX() - clickX) * (unit1.posX() - clickX) +
        (unit1.posY() - clickY) * (unit1.posY() - clickY) -
        (unit2.posX() - clickX) * (unit2.posX() - clickX) -
        (unit2.posY() - clickY) * (unit2.posY() - clickY)
      );
    });
    return units;
  },
    ...
    if (
      ...
      //Override portrait
        ...
        if (Game.selectedUnit instanceof Building)
          $('div.infoLeft div[name="portrait"]')[0].className = Game
            .selectedUnit.attack
            ? Game.selectedUnit.inherited.inherited.name
            : Game.selectedUnit.inherited.name;
      }
      //Show selected unit HP,SP and MP
      $('div.infoLeft span._Health')[0].style.color =
        Game.selectedUnit.lifeStatus();
      ...
      if (Game.selectedUnit.SP) {
        $('div.infoLeft span.shield')[0].innerHTML =
          Game.selectedUnit.shield >> 0;
      ...
      if (Game.selectedUnit.MP) {
        $('div.infoLeft span.magic')[0].innerHTML =
          Game.selectedUnit.magic >> 0;
      ...
      if (Game.selectedUnit.attack) {
        ...
        if (Game.selectedUnit.attackMode) {
          $('div.infoCenter p.damage span')[0].innerHTML =
            Game.selectedUnit.get('attackMode.ground.damage') +
            '/' +
            Game.selectedUnit.get('attackMode.flying.damage');
        } else {
          $('div.infoCenter p.damage span')[0].innerHTML =
            Game.selectedUnit.get('damage') +
            (Game.selectedUnit.suicide ? ' (1)' : '');
        }
      ...
      $('div.infoCenter p.armor span')[0].innerHTML =
        Game.selectedUnit.get('armor');
      if (Game.selectedUnit.get('plasma') != undefined) {
        $('div.infoCenter p.plasma span')[0].innerHTML =
          Game.selectedUnit.get('plasma');
  ...
  draw: function (chara) {
    ...
    //Choose context
    var cxt =
      chara instanceof Unit || chara instanceof Building
        ? Game.cxt
        : Game.frontCxt;
    //Draw shadow
    cxt.save();
      ...
      if (chara.source) imgSrc = sourceLoader.sources[chara.source];
        ...
        imgSrc =
          sourceLoader.sources[
            chara.attack ? chara.inherited.inherited.name : chara.inherited.name
          ];
      }
    }
    //Unit, not building
    ...
    if (chara.selected == true) {
      ...
      cxt.beginPath();
      cxt.arc(
        chara.posX() - Map.offsetX,
        chara.posY() - Map.offsetY,
        chara.radius(),
        0,
        2 * Math.PI
      );
      ...
      if (chara.SP) {
        ...
        cxt.fillRect(
          chara.x - Map.offsetX,
          chara.y - Map.offsetY + offsetY,
          (chara.width * chara.shield) / chara.get('SP'),
          5
        );
        cxt.strokeRect(
          chara.x - Map.offsetX,
        ...
        cxt.fillStyle =
          lifeRatio > 0.7 ? 'green' : lifeRatio > 0.3 ? 'yellow' : 'red'; //Distinguish life
        cxt.fillRect(
          chara.x - Map.offsetX,
          chara.y - Map.offsetY + offsetY + 5,
          chara.width * lifeRatio,
          5
        );
        cxt.strokeRect(
          chara.x - Map.offsetX,
      ...
      if (chara.MP) {
        ...
        cxt.fillRect(
          chara.x - Map.offsetX,
          chara.y - Map.offsetY + offsetY + (chara.SP ? 10 : 5),
          (chara.width * chara.magic) / chara.get('MP'),
          5
        );
        cxt.strokeRect(
          chara.x - Map.offsetX,
  ...
  drawInfoBox: function () {
    ...
    if (
      ...
      $('div.infoLeft span._Health')[0].style.color =
        lifeRatio > 0.7 ? 'green' : lifeRatio > 0.3 ? 'yellow' : 'red';
      $('div.infoLeft span.life')[0].innerHTML = Game.selectedUnit.life >> 0;
      if (Game.selectedUnit.SP) {
        $('div.infoLeft span.shield')[0].innerHTML =
          Game.selectedUnit.shield >> 0;
  ...
  drawSourceBox: function () {
    ...
    $('div.resource_Box span.manNum')[0].style.color =
      Resource[0].curMan > Resource[0].totalMan ? 'red' : '#00ff00';
  },
  drawProcessingBox: function () {
    ...
    if (processing) {
      ...
      var percent =
        ((new Date().getTime() - processing.startTime) / processing.time +
          0.5) >>
        0;
      ...
      $(
        'div.upgrading div[name="processing"] div.processedBar'
  ...
  drawMultiSelectBox: function () {
    ...
    Game.allSelected.forEach(function (chara, N) {
      ...
      if (chara.portrait) node.className = chara.portrait;
        ...
        node.className =
          chara instanceof Building
            ? chara.attack
              ? chara.inherited.inherited.name
              : chara.inherited.name
            : chara.name;
    ...
    $('div.override div.multiSelection').css(
      'width',
      (iconNum > 12 ? Math.ceil(iconNum / 2) * 55 : 330) + 'px'
    );
    //Adjust background position after added into DOM, nth starts from 1st(no 0th)
    for (var n = 1; n <= iconNum; n++) {
      var bgPosition = $(
        'div.override div.multiSelection div:nth-child(' + n + ')'
      ...
      bgPosition = bgPosition
        ...
        .join(' ');
      $('div.override div.multiSelection div:nth-child(' + n + ')').css(
        'background-position',
        bgPosition
      );
    }
    ...
    var loop = function () {
      ...
      for (var N = 0; N < Building.allBuildings.length; N++) {
        ...
        if (build.status == 'dead') {
          if (build.isEnemy) {
            let index = $.inArray(build, Building.enemyBuildings);
            Building.enemyBuildings.splice(index, index == -1 ? 0 : 1);
          } else {
            let index = $.inArray(build, Building.ourBuildings);
      ...
      for (let N = 0; N < Unit.allUnits.length; N++) {
        ...
        if (chara.status == 'dead') {
          if (chara.isFlying) {
            if (chara.isEnemy) {
              let index = $.inArray(chara, Unit.enemyFlyingUnits);
              Unit.enemyFlyingUnits.splice(index, index == -1 ? 0 : 1);
            } else {
              let index = $.inArray(chara, Unit.ourFlyingUnits);
              Unit.ourFlyingUnits.splice(index, index == -1 ? 0 : 1);
            }
          } else {
            if (chara.isEnemy) {
              let index = $.inArray(chara, Unit.enemyGroundUnits);
              Unit.enemyGroundUnits.splice(index, index == -1 ? 0 : 1);
            } else {
              let index = $.inArray(chara, Unit.ourGroundUnits);
      ...
      for (let N = 0; N < Burst.allEffects.length; N++) {
        ...
        if (
          effect.status == 'dead' ||
          (effect.target && effect.target.status == 'dead')
        ) {
      ...
      if (mouseController.drag) {
        ...
        Game.cxt.strokeRect(
          mouseController.startPoint.x,
          mouseController.startPoint.y,
      ...
      if (
        Game.selectedUnit.status == 'dead' ||
        (Game.selectedUnit.isInvisible && Game.selectedUnit.isEnemy)
      ) {
  ...
  resizeWindow: function () {
    ...
    Map.insideStroke.width =
      ((130 * Game.HBOUND) / Map.getCurrentMap().width) >> 0;
    Map.insideStroke.height =
      ((130 * Game.VBOUND) / Map.getCurrentMap().height) >> 0;
    //Redraw map
    Map.draw();
