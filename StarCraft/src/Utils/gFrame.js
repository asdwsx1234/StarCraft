...
var _$ = function (selector) {
  ...
  for (var N = 0; N < selectors.length; N++) {
    ...
    if (filterIndex != -1) {
      filter = curSelector
        .substring(filterIndex + 1, curSelector.indexOf(']'))
        .trim();
    ...
    if (curSelector.includes('#')) {
        ...
        if (result.length) {
          ...
          for (let M = 0; M < result.length; M++) {
            _result = _result.concat(
              $.makeArray(result[M].getElementsByTagName(TagName))
            );
          }
          tagResult = _result;
      ...
      if (className) {
        if (result.length) {
          ...
          for (let M = 0; M < result.length; M++) {
            _result = _result.concat(
              $.makeArray(result[M].getElementsByClassName(className))
            );
          }
          classResult = _result;
        } else
...
_$.declare = function (globalName, fathers, plusObj) {
  ...
  var child = _$.extends(fathers, {
    constructorPlus: constructPlus,
    prototypePlus: protoPlus,
  });
