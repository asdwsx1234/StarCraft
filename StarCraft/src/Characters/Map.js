...
var Map = {
  ...
  refreshFog: function () {
    if (Map.fogFlag && Map.fogCxt && Map.miniFogCxt) {
      ...
      //Clear fog
      Map.fogCxt.globalCompositeOperation =
        Map.miniFogCxt.globalCompositeOperation = 'destination-out';
      //Draw fog
      Unit.allOurUnits()
        ...
        .forEach(function (chara) {
          ...
          var offsetY =
            ((chara.posY() * 130) / Map.getCurrentMap().height) >> 0;
          var sight =
            ((chara.get('sight') * 130) / Map.getCurrentMap().height) >> 0;
          Map.miniFogCxt.beginPath();
          if (Map.fogType) {
            Map.miniFogCxt.drawImage(
              Map.shadowCanvas,
              0,
              0,
              100,
      ...
      Map.fogCxt.globalCompositeOperation =
        Map.miniFogCxt.globalCompositeOperation = 'source-over';
    }
  },
  drawFog: function () {
    if (Map.fogFlag && Map.fogCxt && Map.miniFogCxt) {
  ...
  drawMud: function () {
    ...
    var mudPattern = Game.backCxt.createPattern(
      sourceLoader.sources['Mud'],
      'repeat'
    );
    ...
    Building.allBuildings
      .filter(function (chara) {
        return (
          chara instanceof Building.ZergBuilding &&
          !chara.noMud &&
          chara.insideScreen()
        );
      })
  ...
  draw: function () {
    ...
    Game.backCxt.drawImage(
      Map.getCurrentMap(),
      Map.offsetX,
      Map.offsetY,
      Game.HBOUND,
      Game.VBOUND - Game.infoBox.height + 5,
      0,
      0,
      Game.HBOUND,
      Game.VBOUND - Game.infoBox.height + 5
  ...
  refresh: function (direction) {
    ...
    var edgeY =
      Map.getCurrentMap().height - Game.VBOUND + Game.infoBox.height - 5;
    var onlyMap;
    switch (direction) {
      case 'LEFT':
  ...
  refreshMiniMap: function () {
    ...
    Map.miniCxt.strokeRect(
      ((130 * Map.offsetX) / mapWidth) >> 0,
      ((130 * Map.offsetY) / mapHeight) >> 0,
      Map.insideStroke.width,
      Map.insideStroke.height
    );
  },
  clickHandler: function (event) {
    ...
    //Relocate map center
    Map.relocateAt(
      (Map.getCurrentMap().width * clickX) / 130,
      (Map.getCurrentMap().height * clickY) / 130
    );
  },
