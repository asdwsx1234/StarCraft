...
export class Unit extends GlobalObject {
  ...
  static getSpeedMatrixBy = function (speed) {
  ...
  static walkAround = function () {
    ...
    this.dockTimer = setInterval(function () {
      ...
      if (myself.status == 'dock') {
        myself.moveTo(
          myself.posX() + myself.get('speed')[direction].x * 6,
          myself.posY() + myself.get('speed')[direction].y * 6
        );
      } else {
        clearInterval(myself.dockTimer);
      }
  ...
  //Dock action IV
  static walkAroundLarva = function () {
    //Inherited dock from Unit.js
    Unit.prototype.dock.call(this);
    //Add in new things
    if (this.dockTimer) clearInterval(this.dockTimer);
  ...
  navigateTo(clickX, clickY, range) {
    ...
    if (
      this.insideCircle({ centerX: clickX, centerY: clickY, radius: range })
    ) {
      this.dock();
      //Stop routing
      clearInterval(this.routingTimer);
  ...
  faceTo(target, preventAction) {
    ...
    if (target instanceof GlobalObject) {
      angle = Math.atan(
        (this.posY() - target.posY()) / (target.posX() - this.posX())
      );
    } else {
      //Location={x:1,y:2}
  ...
  escapeFrom(enemy) {
    ...
    this.moveTo(
      this.posX() + escapeSpeed.x * escapeSteps,
      this.posY() + escapeSpeed.y * escapeSteps
    );
  }
    ...
    if (enemyObj instanceof GlobalObject) {
      ...
      if (!enemyAttackType && enemyObj.attackMode) {
        enemyAttackType = this.isFlying
          ? enemyObj.attackMode.flying.attackType
          : enemyObj.attackMode.ground.attackType;
      }
      damage =
        enemyObj.get('damage') *
        Unit.attackMatrix[enemyAttackType][this.unitType];
  ...
  getDamageBy(enemy, percent) {
    ...
    if (this.shield > 0) {
      ...
      damage =
        ((this.calculateDamageBy(enemy) - this.get('armor')) * percent) >> 0;
      if (damage < 1) damage = 0.5;
      this.life -= damage;
    }
  }
...
export class AttackableUnit extends Unit {
  ...
  matchAttackLimit(enemy) {
    ...
    if (this.attackLimit) {
      ...
      if (
        (this.attackLimit == 'flying' && !enemy.isFlying) ||
  ...
  attack(enemy) {
    ...
    if (enemy instanceof GlobalObject && enemy.status != 'dead') {
      ...
      if (this.attackMode) {
        this.Bullet = enemy.isFlying
          ? this.attackMode.flying.Bullet
          : this.attackMode.ground.Bullet;
        this.attackRange = enemy.isFlying
          ? this.get('attackMode.flying.attackRange')
          : this.get('attackMode.ground.attackRange');
        this.attackEffect = enemy.isFlying
          ? this.attackMode.flying.attackEffect
          : this.attackMode.ground.attackEffect;
        this.attackInterval = enemy.isFlying
          ? this.get('attackMode.flying.attackInterval')
          : this.get('attackMode.ground.attackInterval');
        //Change attack bgm
        this.sound.attack = enemy.isFlying
          ? this.sound.attackF
          : this.sound.attackG;
        this.damage = enemy.isFlying
          ? this.get('attackMode.flying.damage')
          : this.get('attackMode.ground.damage');
        this.attackType = enemy.isFlying
          ? this.attackMode.flying.attackType
          : this.attackMode.ground.attackType;
    ...
    var _getDelay = function (chara) {
      ...
      if (chara.attack) {
        ...
        if (chara.matchAttackLimit(myself))
          delay +=
            (chara.life + chara.SP ? chara.shield : 0) /
            chara.calculateDamageBy(myself);
        else delay += 32;
      } else delay += 64;
      return delay;
    };
  ...
  isAttacking() {
    //Has target
    return this.target instanceof GlobalObject;
  }
  followEnemy() {
  ...
  isReloaded() {
    ...
    if (this.bullet instanceof GlobalObject && this.bullet.status != 'dead')
      return false;
