...
const Button = {
  ...
  equipButtonsFor: function (chara) {
        ...
        if (chara.items[N] != null) {
          ...
          switch (chara.items[N].name) {
            ...
            case 'Scarab':
              ...
              break;
            case 'Interceptor':
              $('button[num="' + N + '"]')[0].innerHTML =
                chara.continuousAttack.count;
              break;
          }
        } else {
          $('button[num="' + N + '"]')
        ...
        $('button.' + magic).on('click', function () {
          Unit.allOurUnits()
            ...
            .forEach(function (chara) {
              var duration = Resource.getCost(magic)
                ? Resource.getCost(magic).time
                : 0;
              ...
              if (duration) {
                ...
                if (Magic[magic].credit)
                  Resource.creditBill = Resource.getCost(magic);
                //Payment: chara paypal cost
                if (Resource.paypal.call(chara, Resource.getCost(magic))) {
                  Magic[magic].spell.call(chara);
                }
        ...
        unitTypes.forEach(function (unitType) {
          ...
          if (exceptions.indexOf(unitType) == -1) {
            $('button.' + unitType).on('click', function () {
              ...
              if (Resource.paypal(Resource.getCost(unitType))) {
                if (
                  Resource.getCost(unitType) &&
                  Resource.getCost(unitType).time
                ) {
                  var owner = Game.selectedUnit;
                  var duration = Resource.getCost(unitType).time;
                  //Cheat: Operation cwal
                ...
                .forEach(function (chara) {
                  ...
                  if (Resource.paypal(Resource.getCost(unitType))) {
                    if (
                      ...
                      setTimeout(function () {
                        //Evolve
                        var evolved = new Race[unitType]({
                          x: chara.x,
                          y: chara.y,
                        });
                        if (chara.selected) Game.addIntoAllSelected(evolved);
                        if (chara == Game.selectedUnit)
                          Game.changeSelectedTo(evolved);
        ...
        buildNames.forEach(function (buildName) {
          $('button.' + buildName).on('click', function () {
            ...
            if (Resource.paypal(Resource.getCost(buildName))) {
              if (
                Resource.getCost(buildName) &&
                Resource.getCost(buildName).time
              ) {
                var owner = Game.selectedUnit;
                ...
                if (Cheat.cwal) duration = 0;
                setTimeout(function () {
                  var building = new Build[buildName]({
                    x: owner.x,
                    y: owner.y,
                  });
                  if (building instanceof Building.ZergBuilding)
                    setTimeout(Map.drawMud, 0);
                  delete owner.processing;
                }, duration * 100);
                //Occupy flag
                owner.processing = {
    ...
    $('div.panel_Control button').on('mouseover', function (event) {
      ...
      $('div.tooltip_Box')
        .css('right', innerWidth - event.clientX)
        .css('bottom', innerHeight - event.clientY)
        .show();
    ...
    Unit.allOurUnits()
      ...
      .forEach(function (chara) {
        if (chara.hold) {
          ...
          chara.findNearbyTargets =
            Building.Attackable.prototypePlus.findNearbyTargets;
          chara.dock();
          chara.hold = true;
          Button.reset();
        }
    ...
    switch (Button.callback) {
      ...
      default:
        if (typeof Button.callback == 'function') {
          ...
          const offset = $('#frontCanvas').offset();
