  ...
  Unburrow: {
    ...
    spell: function () {
      ...
      setTimeout(function () {
        ...
        myself.dock();
        myself.direction =
          myself.name == 'Hydralisk' || myself.name == 'Lurker' ? 2 : 3;
  ...
  SpawnBroodlings: {
    ...
    spell: function (location) {
      ...
      if (location) {
        ...
        var target = Game.getSelectedOne(
          location.x,
          location.y,
          true,
          true,
          ...
          if (Resource.payCreditBill.call(myself)) {
            ...
            bullet.fire(function () {
              ...
              if (anime.insideScreen())
                new Audio('bgm/Magic.Ensnare.wav').play();
          ...
          this.moveToward(target, 70, function () {
            ...
            var anime = new Animation.Consume({
              ...
              callback: function () {
                //Consume sound
                if (anime.insideScreen())
                  new Audio('bgm/Magic.Consume.wav').play();
            ...
            if (Resource.payCreditBill.call(myself)) {
              ...
              bullet.fire(function () {
                ...
                if (target.status != 'dead') {
                  ...
                  //Lockdown sound
                  if (anime.insideScreen())
                    new Audio('bgm/Magic.Lockdown.wav').play();
                }
              });
            }
          });
      ...
      if (location) {
        ...
        this.moveTo(location.x, location.y, this.get('sight'), function () {
          ...
          bullet.fire(function () {
            ...
            //Nuclear sound
            if (anime.insideScreen())
              new Audio('bgm/Magic.NuclearStrike.wav').play();
            //Use one bomb
            ...
            var healTimer = setInterval(function () {
              ...
              if (myself.magic && target.life < target.get('HP')) {
                ...
                if (myself.insideScreen())
                  new Audio('bgm/Magic.Heal.wav').play();
        ...
        if (target instanceof Gobj) {
          ...
          this.moveToward(target, 250, function () {
            if (Resource.payCreditBill.call(myself)) {
              ...
              if (anime.insideScreen())
                new Audio('bgm/Magic.DefensiveMatrix.wav').play();
        ...
        this.moveTo(location.x, location.y, this.get('sight'), function () {
          if (Resource.payCreditBill.call(myself)) {
            ...
            bullet.fire(function () {
              ...
              //EMPShockwave sound
              if (anime.insideScreen())
                new Audio('bgm/Magic.EMPShockwave.wav').play();
      ...
      if (location) {
        ...
        if (target instanceof Gobj) {
          ...
          var irradiate = function (chara) {
            ...
            //Irradiate sound
            if (anime.insideScreen())
              new Audio('bgm/Magic.Irradiate.wav').play();
        ...
        if (target instanceof Gobj) {
          ...
          this.moveToward(target, this.get('sight'), function () {
            if (Resource.payCreditBill.call(myself)) {
              ...
              bullet.fire();
              if (myself.insideScreen())
                new Audio('bgm/HeroCruiser.attack.wav').play();
            }
          });
        }
        //Empty object {}, cannot spell
    ...
    spell: function (location) {
      ...
      if (location) {
        if (Resource.payCreditBill.call(this)) {
          ...
          //ScannerSweep sound
          if (anime.insideScreen())
            new Audio('bgm/Magic.ScannerSweep.wav').play();
        }
      }
      //If missing location info, mark Button.callback, mouseController will call back with location
      else {
      ...
      if (location) {
        ...
        this.moveTo(location.x, location.y, this.get('sight'), function () {
          if (Resource.payCreditBill.call(myself)) {
            ...
            //PsionicStorm sound
            if (anime.insideScreen())
              new Audio('bgm/Magic.PsionicStorm.wav').play();
            //PsionicStorm effect
            var targets = [];
            ...
            var stormWave = function () {
              ...
              if (psionicStorms.length) {
                ...
                targets.forEach(function (chara) {
                  ...
                  chara.reactionWhenAttackedBy(
                    Magic.PsionicStorm.speller,
                    true
                  );
                });
        ...
        if (target instanceof Gobj) {
          ...
          this.moveToward(target, 245, function () {
            if (Resource.payCreditBill.call(myself)) {
              ...
              for (var n = 0; n < 2; n++) {
                var hallucination = new halluConstructor({
                  x: target.posX(),
                  y: target.posY(),
                });
        ...
        if (target instanceof Gobj) {
          ...
          this.moveToward(target, 280, function () {
            if (Resource.payCreditBill.call(myself)) {
              ...
              if (target.isFlying) {
                Unit.enemyFlyingUnits.splice(
                  Unit.enemyFlyingUnits.indexOf(target),
                  1
                );
                Unit.ourFlyingUnits.push(target);
              } else {
                Unit.enemyGroundUnits.splice(
                  Unit.enemyGroundUnits.indexOf(target),
                  1
                );
          ...
          if (Resource.payCreditBill.call(myself)) {
            ...
            var anime = new Animation.MaelStormSpell({
              ...
              callback: function () {
                ...
                //MaelStorm sound
                if (anime.insideScreen())
                  new Audio('bgm/Magic.MaelStorm.wav').play();
              },
            });
            //MaelStormSpell sound
            if (anime.insideScreen())
              new Audio('bgm/Magic.StasisField.wav').play();
          }
        });
      }
      //If missing location info, mark Button.callback, mouseController will call back with location
          ...
          var anime = new Animation.Recall({
            ...
            callback: function () {
              ...
              if (animeII.insideScreen())
                new Audio('bgm/Magic.Recall.wav').play();
              //Effect
              targets.forEach(function (chara) {
                //Relocate targets
                chara.x = myself.x;
      ...
      if (location) {
        ...
        this.moveTo(location.x, location.y, this.get('sight'), function () {
          if (Resource.payCreditBill.call(myself)) {
            ...
            //DisruptionWeb sound
            if (anime.insideScreen())
              new Audio('bgm/Magic.DisruptionWeb.wav').play();
            //Dynamic update targets every 1 second
            var targets = [];
